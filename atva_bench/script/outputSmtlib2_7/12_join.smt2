...
(assert (str.in_re original is_str_re))
(declare-const split_seq String)
(assert (str.in_re split_seq is_seq_re))
(declare-const rejoined String)
(assert (str.in_re rejoined is_str_re))

(assert (= original "a,b,c"))
(assert (= split_seq (str_split original ",")))
(assert (= rejoined (seq_join split_seq ",")))
