...
(assert (str.in_re s2 is_seq_re))

(assert (= s (str.++ seq_spliter "a")))               ; Initial sequence: ["a"]
(assert (= s2 (seq_write s 0 "b")))          ; Write "b" at index 0

(assert (= (seq_nth s2 0) "b"))              ; Check result
