...
(assert (str.in_re s is_seq_re))
(declare-const digits String)
(assert (str.in_re digits is_seq_re))

; Define regex to match digits
(define-fun digit_re () RegLan (re.range "0" "9"))

(assert (= s (str.++ (str.++ seq_spliter "a") (str.++ seq_spliter "5") (str.++ seq_spliter "z") (str.++ seq_spliter "2"))))

(assert (= digits (seq.filter s digit_re))) ; Should keep "5" and "2"

(assert (= (seq_nth digits 0) "5"))
(assert (= (seq_nth digits 1) "2"))
