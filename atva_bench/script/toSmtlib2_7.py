...
parser.add_argument("-f", "--filename", type=str, help="The file to translate")
...
parser.add_argument("-i", "--input", type=str, help="The directory to save the input file", default="./input")
...
def translateSingleFile(file: str) -> None:
  ...
  with open(file, 'r') as f:
    smtStr = f.read()
  # Translate the file
  smtStr = translate(smtStr)
  print(f"Translation complete for {file}, the result is:\n{smtStr}")
  
def translateAllFiles(inputDir: str) -> None:
  ...
  for file in files:
    ...
    fileCount += 1
    output_file = os.path.join(args.output, filname)
