...
(define-fun r () RegLan
  (re.inter
    (re.++ (re.+ (re.range "a" "z")) (str.to_re ".") (re.+ (re.range "0" "9")))
    (re.union (re.++ re.all (str.to_re "#")) (re.++ (re.* (re.range "0" "9")) (str.to_re ".")))
  )
)
...
(assert (str.in_re str (re.++ (re.+ (re.range "0" "9")) r (re.+ (re.range "a" "z")))))
