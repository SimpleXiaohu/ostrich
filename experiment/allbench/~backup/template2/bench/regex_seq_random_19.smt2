...
(define-fun n () RegLan
  (re.++ (re.+ (re.range "a" "f")) (re.++ (str.to_re ":") (re.+ (re.range "0" "9"))))
)

(define-fun r () RegLan
  (re.inter
    (re.++ (re.+ (re.union (re.range "a" "f") (re.range "0" "9"))) n)
    (re.union (re.++ re.all (str.to_re "#") n) (re.++ (re.* (re.range "0" "9")) (str.to_re ".") (re.* (re.range "0" "9"))))
  )
)
...
(assert (str.in_re z (re.++ (re.+ (re.range "a" "f")) r (re.* (str.to_re "_")))))
