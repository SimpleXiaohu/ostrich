##=== (seq.write seq i str)
# (define-fun seq_write ((s (Seq String)) (i Int) (str String)) (Seq String) 
#   (ite (or (< i 0) (>= i (seq.len s))) s (seq.++ (seq.extract s 0 i) t (seq.extract s (+ i 1) (- (seq.len s) (+ i 1)))))
# )
...
def addSeqWriteHeader(smtStr: str) -> str :
  seqWriteHeader = (
    ...
    "  (ite (or (< i 0) (>= i (seq.len seq))) seq (seq.++ (seq.extract seq 0 i) (seq.unit str) (seq.extract seq (+ i 1) (- (seq.len seq) (+ i 1)))))\n",
    ")\n"
  )
  return "".join(seqWriteHeader) + smtStr
    

def ostrichseq2z3seq(smtStr: str) -> str :
...
def translateSingleFile(file: str) -> None:
  with open(file, 'r') as f:
    smtStr = f.read()
  # Translate the file
  smtStr = ostrichseq2z3seq(smtStr)
  print(smtStr)
