...
(assert (str.in_re text is_str_re))
(declare-const letters String)
(assert (str.in_re letters is_seq_re))

(assert (= text "XyZabc123"))
(define-fun lowercase () RegLan (re.range "a" "z"))
(assert (= letters (str.match_all text lowercase)))

(assert (= (seq_nth letters 0) "y"))
(assert (= (seq_nth letters 1) "a"))
(assert (= (seq_nth letters 2) "b"))
(assert (= (seq_nth letters 3) "c"))
