...
(assert (str.in_re raw is_str_re))
(declare-const chunks String)
(assert (str.in_re chunks is_seq_re))
(declare-const trimmed String)
(assert (str.in_re trimmed is_seq_re))
(declare-const result String)
(assert (str.in_re result is_str_re))

(assert (= raw "foo, ,bar"))
(assert (= chunks (str_split raw ",")))

(define-fun non_empty () RegLan (re.+ (re.union (re.range "a" "z") (re.range "A" "Z"))))
(assert (= trimmed (seq.filter chunks non_empty)))
(assert (= result (seq_join trimmed ",")))
