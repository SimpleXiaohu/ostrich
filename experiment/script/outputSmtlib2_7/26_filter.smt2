...
(assert (str.in_re logs is_seq_re))
(declare-const errors String)
(assert (str.in_re errors is_seq_re))

(assert (= logs
  (str.++ (seq.unit "INFO start")
          (seq.unit "ERROR failed to open file")
          (seq.unit "DEBUG checking")
          (seq.unit "ERROR timeout"))))

(assert (= errors (seq.filter logs (re.++ (str.to_re "ERROR") (re.* re.allchar)))))
(assert (= (seq_len errors) 2))
