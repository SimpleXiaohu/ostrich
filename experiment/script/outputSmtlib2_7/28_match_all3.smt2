...
(assert (str.in_re s is_str_re))
(declare-const nums String)
(assert (str.in_re nums is_seq_re))

(assert (= s "a1b22c333"))
(define-fun digit_re () RegLan (re.+ (re.range "0" "9")))
(assert (= nums (str.match_all s digit_re)))

(assert (= (seq_nth nums 0) "1"))
(assert (= (seq_nth nums 1) "22"))
(assert (= (seq_nth nums 2) "333"))
