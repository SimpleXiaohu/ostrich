...
(assert (str.in_re filename is_str_re))
(declare-const parts String)
(assert (str.in_re parts is_seq_re))
(declare-const ext String)
(assert (str.in_re ext is_str_re))

(assert (= filename "report.final.pdf"))
(assert (= parts (str_split filename ".")))
(assert (= ext (seq_nth parts (- (seq_len parts) 1))))
(assert (= ext "pdf"))
