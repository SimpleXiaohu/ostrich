...
(assert (str.in_re numbers is_seq_re))

(assert (= input (str.++ (str.++ seq_spliter "123") (str.++ seq_spliter "abc") (str.++ seq_spliter "456"))))
(define-fun digits () RegLan (re.+ (re.range "0" "9")))
(assert (= numbers (seq.filter input digits)))
(assert (= (seq_nth numbers 0) "123"))
(assert (= (seq_nth numbers 1) "456"))
