...
from ostrichseq2string import ostrichseq2string
...
class Z3Interface(SolverInterface):
    def convert_input(self, benchmark_path: str) -> str:
        with open(benchmark_path, "r") as f:
            content = f.read()
        tmp = tempfile.NamedTemporaryFile(mode="w", delete=False, suffix=".smt2")
        tmp.write(ostrichseq2string(content))
        tmp.close()
        return tmp.name
...
def run_all(solvers, interfaces, benchmarks, db_path="results.db"):
    ...
    with ThreadPoolExecutor() as executor:
        ...
        for solver_name, solver_info in solvers.items():
            ...
            for bench in benchmarks:
                futures.append(
                    executor.submit(
                        run_solver, solver_name, solver_info, interface, bench
                    )
                )
    ...
    for benchmark, solver, status in cur.execute(
        "SELECT benchmark, solver, status FROM results"
    ):
    ...
    for row in cur.execute(
        "SELECT solver, status, COUNT(*) FROM results GROUP BY solver, status"
    ):
...
if __name__ == "__main__":
    ...
    benchmark_dir = script_root / "../allbench/bench_base/template3"
