...
object SeqExtractCEPreOp {
  def apply(offset: ITerm, length: ITerm, globalArith: Formula): SeqExtractCEPreOp =
    new SeqExtractCEPreOp(offset, length, globalArith)
}
...
class SeqExtractCEPreOp(off: ITerm, len: ITerm, globalArith: Formula) extends CEPreOp {
  ...
  def apply(
    ...
    val emptyResPreimage = if (res.isAccept(res.initialState)) {
      ...
      var finalEmptyResPreImage: Iterator[Seq[Automaton]] = Iterator()
      SimpleAPI.withProver { p =>
        ...
        if (isSat(preimage3.regsRelation)) {
          finalEmptyResPreImage = finalEmptyResPreImage ++ Iterator(Seq(preimage3))
        }
        if (isSat(preimage4.regsRelation)) {
          finalEmptyResPreImage = finalEmptyResPreImage ++ Iterator(Seq(preimage4))
        }
      }
      finalEmptyResPreImage
    } else {
      Iterator()
    }
  ...
  def eval(arguments: Seq[Seq[Int]]): Option[Seq[Int]] = {
    ...
    Some(
      subSeq.foldLeft(Seq(StringSeqAutomaton.seqSplitter)) { case (splitter, seq) =>
        splitter ++ seq :+ StringSeqAutomaton.seqSplitter
      }
    )
  }
}
