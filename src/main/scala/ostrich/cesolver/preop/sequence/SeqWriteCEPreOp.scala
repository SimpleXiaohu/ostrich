...
class SeqWriteCEPreOp(i: ITerm) extends CEPreOp {
  ...
  /** The automaton that accepts string sequences of length 1 and the element is all strings in the alphabet.
    */
  private def oneAnyStrSeqAut(registers: Seq[ITerm] = Seq(), firstUpdate: Seq[Int] = Seq()): StringSeqAutomaton = {
    val res         = new StringSeqAutomaton
    val emptyUpdate = Seq.fill(registers.length)(0)
    val acceptedS   = res.newState()
    res.addSeqElementConnect(res.initialState, acceptedS, firstUpdate)
    res.addTransition(acceptedS, res.LabelOps.sigmaLabel, acceptedS, emptyUpdate)
    res.setAccept(acceptedS, true)
    res.registers = Seq.fill(registers.length)(TermGenerator().registerTerm)
    res
  }
      ...
      for (s <- after.states) yield {
        ...
        if (pre.isEmpty | in.isEmpty | after.isEmpty) Seq()
          ...
          for ((_, v) <- in.nextSeqElements(in.initialState))
            firstUpdate = v
          val preImageIn = oneAnyStrSeqAut(in.registers, firstUpdate)
          ...
          val updateResRegisterF = connectSimplify(
            (preRegisters zip strIn.registers zip res.registers).map { case ((reg1, reg2), res) => reg1 + reg2 === res },
            IBinJunctor.And
          )
