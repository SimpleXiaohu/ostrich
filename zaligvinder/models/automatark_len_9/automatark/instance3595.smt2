...
; (LT-?)?([0-9]{9}|[0-9]{12})
(assert (not (str.in_re X (re.++ (re.opt (re.++ (str.to_re "LT") (re.opt (str.to_re "-")))) (re.union ((_ re.loop 9 9) (re.range "0" "9")) ((_ re.loop 12 12) (re.range "0" "9"))) (str.to_re "\u{0a}")))))
...
; ^(\{|\[|\().+(\}|\]|\)).+$
(assert (str.in_re X (re.++ (re.union (str.to_re "{") (str.to_re "[") (str.to_re "(")) (re.+ re.allchar) (re.union (str.to_re "}") (str.to_re "]") (str.to_re ")")) (re.+ re.allchar) (str.to_re "\u{0a}"))))
(assert (> (str.len X) 9))
(check-sat)
